...
Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively (no user available to choose), write plans for the top 3–5 by leverage and record that default in the selected plan root's `README.md`.

### Phase 4 — Write the plans

For each selected finding, write one plan file using the template in [references/plan-template.md](references/plan-template.md) — read it before writing the first plan. Select the plan root once for this run: use `plans/` by default; use `advisor-plans/` only when an existing `plans/` directory is unrelated. Create the selected directory if absent. This selected root is the shared contract for every plan path, dependency, and index reference in this skill and its references. Plans go in:
...
Before writing anything: record `git rev-parse --short HEAD` — every plan stamps the commit it was written against (the executor uses it for drift detection). If the selected plan root already exists from a previous run, **reconcile, don't duplicate**: read its `README.md`, keep numbering monotonic, skip findings already planned or listed as rejected, and mark superseded plans stale in the index.
...
Finish by writing the selected plan root's `README.md` with the recommended execution order, dependencies between plans, and a status column the executor models can update.
