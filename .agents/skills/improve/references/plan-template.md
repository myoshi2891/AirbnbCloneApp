...
`<plan-root>` means the selected plan root defined in the improve skill: `plans/` by default, or `advisor-plans/` only when `plans/` is unrelated. File naming: `<plan-root>/NNN-short-slug.md`, numbered in recommended execution order.
...
Before writing this plan, inspect the repository and replace every placeholder below with the verified repository-specific command. Do not guess commands or leave placeholders in a completed plan.

| Purpose   | Command                          | Expected on success |
|-----------|----------------------------------|---------------------|
| Install   | `<verified install command>`      | exit 0              |
| Typecheck | `<verified typecheck command>`    | exit 0, no errors   |
| Tests     | `<verified test command/filter>`  | all pass            |
| Lint      | `<verified lint command>`         | exit 0              |

(Each command must be verified during recon.)
...
- [ ] `<verified typecheck command>` exits 0
- [ ] `<verified test command>` exits 0; new tests for <X> exist and pass
