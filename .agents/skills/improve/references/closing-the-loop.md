...
- The plan file exists and its dependencies show DONE in the selected plan root's `README.md`. If not: stop, name the missing dependency.
...
1. **The full plan file text, inlined.** The worktree contains only committed files — if the selected plan root is uncommitted, the executor can't read it. Never assume; always inline.
...
> One override: SKIP the plan's instruction to update the selected plan root's
> `README.md` —
...
## `reconcile` — keep the selected plan root alive

Process what happened since the last session. Read the selected plan root's `README.md` and every plan file, then per status:
